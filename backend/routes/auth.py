...
def create_access_token(data: dict) -> str:
    ...
    encoded_jwt = jwt.encode(to_encode, getattr(settings, 'JWT_SECRET', 'dev-secret-key-123'), algorithm="HS256")
    return encoded_jwt
...
def get_user_from_session(token: str) -> Optional[dict]:
    ...
    try:
        return jwt.decode(token, getattr(settings, 'JWT_SECRET', 'dev-secret-key-123'), algorithms=["HS256"])
    except:
        return None
